...
async function refreshBatches(preferredId = selectedBatchId.value) {
  batches.value = await listPhotoBatches(user.value.id)
...
watch(selectedPhoto, photo => {
  ...
  showManualSearch.value = Boolean(photo && (photo.status === 'error' || (photo.status === 'review' && !photo.itemId)))
})
...
async function processQueue() {
  if (processing.value || currentBatch.value?.status !== 'pending') return
...
async function confirmBatch() {
  if (!currentBatch.value || blockReason.value) return
  saving.value = true
  try {
  ...
  <div class="space-y-4 pb-24 lg:pb-4">
    <div class="flex flex-wrap items-start justify-between gap-3 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
      <div>
        ...
        <p class="mt-1 max-w-2xl text-xs text-gray-500 dark:text-gray-400">Os lotes são sincronizados com sua conta. A IA sugere; você sempre revisa e confirma.</p>
      </div>
    ...
    <p v-if="loading" class="rounded-lg border border-gray-200 bg-white px-4 py-8 text-center text-sm text-gray-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400">Carregando lotes deste navegador…</p>
        ...
        <section class="rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
          ...
          <div v-if="completedBatches.length" class="mt-2 space-y-1.5">
            <button v-for="batch in completedBatches" :key="batch.id" type="button" class="flex min-h-11 w-full items-center justify-between gap-3 rounded-lg border px-3 py-2 text-left transition-colors" :class="selectedBatchId === batch.id ? 'border-green-400 bg-green-50 text-green-800 dark:border-green-800 dark:bg-green-950/25 dark:text-green-300' : 'border-gray-200 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800'" @click="selectedBatchId = batch.id">
              <span><span class="block text-sm font-semibold">{{ batchTitle(batch) }}</span><span class="block text-[11px] opacity-70">Concluído em {{ formatDate(batch.completedAt) }}</span></span>
      ...
      <div v-if="currentBatch" class="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
        <header class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 p-4 dark:border-gray-700">
          <div>
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ photos.length }} foto(s)<span v-if="queueCount"> · {{ queueCount }} na fila</span></p>
          </div>
          <div v-if="currentBatch.status === 'pending'" class="flex flex-wrap items-center justify-end gap-2">
            <ConfirmInline v-if="deleteBatchId === currentBatch.id" message="Excluir lote e fotos deste navegador?" confirm-label="Excluir" cancel-label="Cancelar" @confirm="removeBatch(currentBatch.id)" @cancel="deleteBatchId = ''" />
            <AppButton v-else variant="ghost" size="sm" @click="deleteBatchId = currentBatch.id">Excluir lote</AppButton>
            <label class="ds-button ds-button-secondary ds-button-sm inline-flex cursor-pointer items-center justify-center gap-2 focus-within:ring-2 focus-within:ring-primary-400">
              <input class="sr-only" type="file" multiple accept="image/*" @change="onFilesSelected" />
              Escolher várias fotos
            </label>
            <label class="ds-button ds-button-primary ds-button-md inline-flex cursor-pointer items-center justify-center gap-2 focus-within:ring-2 focus-within:ring-primary-400" style="color: var(--ds-primary-text)">
        ...
        <div v-if="photos.length" class="grid min-h-[520px] lg:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.35fr)]">
          ...
          <main v-if="selectedPhoto" class="min-w-0 p-4">
            ...
            <div v-if="selectedPhoto.status === 'error' && currentBatch.status === 'pending'" class="mt-2"><AppButton variant="secondary" size="sm" @click="retryAnalysis(selectedPhoto)">Tentar análise novamente</AppButton></div>
            ...
            <section class="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800/50">
              <div class="flex flex-wrap items-center justify-between gap-2">
                ...
                <AppButton v-if="currentBatch.status === 'pending'" variant="ghost" size="xs" @click="showManualSearch = !showManualSearch">{{ showManualSearch ? 'Fechar busca' : 'Buscar manualmente' }}</AppButton>
              </div>
              ...
              <div v-if="currentItem && !showManualSearch" class="mt-3">
                ...
                <div v-else-if="currentItemVariations.length" class="mt-3">
            ...
            <section class="mt-4 grid gap-3 sm:grid-cols-2">
              <label class="block"><span class="mb-1 block text-xs font-semibold text-gray-600 dark:text-gray-300">Quantidade</span><input v-model="selectedPhoto.qty" type="number" min="1" step="1" :disabled="currentBatch.status !== 'pending'" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm tabular-nums text-gray-900 outline-none focus:border-primary-400 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100" @change="savePhoto(selectedPhoto)" /></label>
              <label v-if="currentBatch.type === 'entrada'" class="block"><span class="mb-1 block text-xs font-semibold text-gray-600 dark:text-gray-300">Custo unitário</span><input v-model="selectedPhoto.unitCost" type="number" min="0" step="0.01" :disabled="currentBatch.status !== 'pending'" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm tabular-nums text-gray-900 outline-none focus:border-primary-400 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100" placeholder="Opcional" @change="savePhoto(selectedPhoto)" /></label>
            ...
            <section v-if="currentBatch.status === 'pending'" class="mt-4 rounded-lg border border-gray-200 p-3 dark:border-gray-700">
            ...
            <div v-if="currentBatch.status === 'pending'" class="mt-4 flex justify-end">
        ...
        <section v-if="currentBatch.status === 'pending'" class="border-t border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/40">
    ...
    <footer v-if="currentBatch?.status === 'pending'" class="fixed inset-x-0 bottom-0 z-30 border-t border-gray-200 bg-white/95 p-3 backdrop-blur dark:border-gray-700 dark:bg-gray-900/95 lg:sticky lg:inset-x-auto lg:rounded-xl lg:border">
