        ...
        <div v-else-if="!selectedSubcategory" class="p-5">
          <div class="mb-5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
            ...
            <div v-if="categoryDirectItems.length" class="divide-y divide-gray-100 dark:divide-gray-700/50">
              <div v-for="item in categoryDirectItems" :key="item.id" class="px-4 py-3 flex flex-wrap items-center justify-between gap-3">
                <div>
            ...
            <div v-else-if="addingItemForSub !== categoryDirectKey" class="px-4 py-4 text-sm text-gray-400 dark:text-gray-500 italic">Nenhum item direto nesta categoria.</div>
            ...
            <div v-if="addingItemForSub === categoryDirectKey" class="px-4 py-3 bg-primary-50/60 dark:bg-primary-900/10">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <input v-model="newItemName" :placeholder="`Nome do item em ${selectedCategory}`" class="flex-1 min-w-[160px] px-2.5 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 placeholder-gray-300 dark:placeholder-gray-600 focus:outline-none focus:border-primary-400 dark:focus:border-primary-500" @keydown="onNewItemKeydown" autofocus />
                <div class="flex-1 min-w-[180px] flex flex-wrap items-center gap-1 px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus-within:border-primary-400 dark:focus-within:border-primary-500">
                  <span v-for="(attr, i) in newItemAttrs" :key="attr" class="inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[11px] rounded bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300">
                    {{ attr }}
                  ...
                  <input v-model="newItemAttrInput" placeholder="+ atributo" class="flex-1 min-w-[80px] text-xs bg-transparent text-gray-700 dark:text-gray-200 placeholder-gray-300 dark:placeholder-gray-600 focus:outline-none py-0.5" @keydown="onNewItemAttrKeydown" />
                </div>
                ...
                <tr v-if="addingSubcategory" class="bg-primary-50/60 dark:bg-primary-900/10">
                  <td colspan="4" class="px-4 py-3">
                    <div class="flex items-center gap-2 flex-wrap">
                      ...
                      <div class="flex-1 min-w-[180px] flex flex-wrap items-center gap-1 px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus-within:border-primary-400 dark:focus-within:border-primary-500">
                        <span v-for="(attr, i) in newSubcategoryAttrs" :key="attr" class="inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[11px] rounded bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300">
                          {{ attr }}
                          <button class="text-primary-400 hover:text-red-500" @click.stop="newSubcategoryAttrs.splice(i, 1)"><svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg></button>
                        </span>
                        <input v-model="newSubcategoryAttrInput" placeholder="+ modelo de variação" class="flex-1 min-w-[110px] text-xs bg-transparent text-gray-700 dark:text-gray-200 placeholder-gray-300 dark:placeholder-gray-600 focus:outline-none py-0.5" @keydown="onNewSubcategoryAttrKeydown" />
