...
function populateSeed() {
  ...
  for (const r of roles) insertRole.run(r.id, r.name, r.description || '', r.active !== false ? 1 : 0)
  for (const p of people) insertPerson.run(p.id, p.name, p.role || '', p.active !== false ? 1 : 0)
  for (const s of suppliers) insertSupplier.run(s.id, s.name, s.description || '', s.active !== false ? 1 : 0)
  for (const l of locations) insertLocation.run(l.id, l.name, l.description || '', l.active !== false ? 1 : 0, l.parentId || null)
  for (const d of destinations) insertDestination.run(d.id, d.name, d.description || '', d.active !== false ? 1 : 0, d.parentId || null)
  for (const i of seedItems) insertItem.run(i.id, i.name, i.group, i.category || null, i.subcategory || null, i.unit || 'UN', i.minStock || 0, JSON.stringify(i.attributes || []), i.location || '')
  for (const v of seedVars) {
    const stock = v.stock || 0
    insertVar.run(v.id, v.itemId, JSON.stringify(v.values || {}), stock, v.minStock || 0, v.initialStock !== undefined ? v.initialStock : stock, JSON.stringify(v.extras || {}), v.location || '', JSON.stringify(v.destinations || []))
  }
